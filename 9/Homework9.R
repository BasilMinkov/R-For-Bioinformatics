...
n_gens = 200 # just to make procedure faster 

brain_exp = df[c(1:n_gens),c(1:56)]
liver_exp = df[c(1:n_gens),c(57:dim(df)[2])]
...
adj.t.p.v = p.adjust(t.p.v, "BH")

# Here we get p-values after fdr correction
sum1 = sum(adj.t.p.v < 0.05)
sum1

# Here we start the permutations 

n_perm = 20

perm.t.p.v = matrix(0, dim(brain_exp)[1], n_perm)

for (perm in 1:n_perm){
  print(perm)
  s = sample(dim(df)[2])
  p_cont = df[, s[c(1:56)]]
  p_exp = df[, s[c(57:dim(df)[2])]]
  perm.t.p.v[, perm] = StudentGoesHard(p_cont, p_exp)
}

# corresponding p-value threshold
thr = max(adj.t.p.v[adj.t.p.v <= 0.05])
thr

# fdr evaluation
apply(perm.t.p.v<thr,2,sum)
